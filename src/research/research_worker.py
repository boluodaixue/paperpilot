...
def _strong_worker_evidence(
    ...
    allowed_questions = set(packet.question_ids)
    ...
    eligible = tuple(
        ...
        if item.requirement_id in allowed_questions
        and item.action_id
...
def _worker_result(packet: WorkPacket, result: ResearchResult) -> BlueWorkerResult:
    evidence = _strong_worker_evidence(result.evidence, packet)
    claims = tuple(
    ...
    *,
    ...
    if snapshot.values:
        ...
        if isinstance(saved_result, ResearchResult):
            return _worker_result(packet, saved_result)
    ...
    return _worker_result(packet, result)
