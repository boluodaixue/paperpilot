...
每轮工具或子 Agent 结果后，同一张图的 `assess_completion` 节点记录唯一证据、独立来源、本轮信息增量和 Research Brief 中的研究方向。它不是新 Agent、RCS 或质量评分器：

- 达到可解释的来源底线后，先要求当前 Agent 指出具体未覆盖方向；
- 连续两轮满足底线时，禁用工具并要求返回最终结构化结果；
- 连续两轮没有新的可定位证据或来源时，以 `evidence_saturated` 部分完成；
- 工具、token、时间、迭代和递归限制仍作为最后硬停止。

根 Agent 的来源底线根据已确认方向数量在 4–12 之间有界调整，子/孙 Agent 使用更小的局部底线。它只判断是否应当收束，不宣称替代事实准确性、引用覆盖或 LLM-as-Judge 评测。
