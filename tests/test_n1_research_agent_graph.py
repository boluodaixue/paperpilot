...
async def test_large_tool_result_is_trimmed_only_after_verified_artifact_write() -> None:
    ...
    assert store.calls[0]["artifact_scope_id"] == identity.root_thread_id
    tool_message = next(
    ...
    assert '"artifact_read_root": "artifact"' in tool_message
    assert result["result"].evidence


@pytest.mark.asyncio
async def test_l3_consolidation_checkpoints_state_projection_and_semantic_memo() -> None:
    class ConsolidatingPolicy:
        ...
        def __call__(self, messages, *, tools=None):
            content = str(messages[-1].get("content") or "")
            if content.startswith("CONSOLIDATE_RESEARCH_CONTEXT\n"):
                self.memo_calls += 1
                request = json.loads(content.split("\n", 1)[1])
                return {
                    "content": json.dumps(
                        {
                            "summary": "Earlier reasoning remains unresolved.",
                            "requirement_ids": request["required_requirement_ids"],
                            "evidence_ids": request["required_evidence_ids"],
                            "artifact_ids": request["required_artifact_ids"],
                        }
                    )
                }
            return _final("Consolidated.", "No unsupported fact was added.", "partial")

    policy = ConsolidatingPolicy()
    identity = _root_identity("root-l3-consolidation")
    state = _state(
        ResearchTask("l3-consolidation", "Preserve long-running research context."),
        identity,
        AgentLimits(max_iterations=3, max_total_tokens=300000),
    )
    state["messages"] = [
        {"role": "system", "content": "system"},
        {"role": "user", "content": "task"},
        *(
            {"role": "user", "content": f"reasoning-{index} " + ("x" * 4000)}
    ...
    assert policy.memo_calls == 1
    assert final["compression_manifests"]
    assert final["semantic_memos"]
    assert final["compression_manifests"][0]["research_state"]["coverage"]
